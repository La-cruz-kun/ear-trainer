            ...
            if (strncmp (frame_header, frame_id, 4) == 0)
                {
                    unsigned int frame_size
                        = syncsafe_to_int ((unsigned char *)frame_header + 4);
                    ...
                    if (data[0] == 0x03)
                    ...
                    else if (data[0] == 0x01)
                        {
                            ...
                            iconv_t cd = iconv_open (
                                "UTF-8", "UTF-16LE"); // define the conversion
                            size_t in_bytes
                                = frame_size; // the size of the input
                            char *in_buf
                                = (char *)&data[3]; // the input itself starts
                                                    // at index 3 because we
                                                    // skip both the encoding
                                                    // info and the BOM
                            char out_buf[100];
                            char *out_ptr = out_buf;
                            size_t out_bytes = sizeof (out_buf);

                            iconv (cd, &in_buf, &in_bytes, &out_ptr,
                                   &out_bytes); // do the conversion
                            iconv_close (cd);
