...
void DrawMenuButton(MenuButton button) {
    NPatchInfo ninePatchInfo1 = { (Rectangle){ 0.0f, 0.0f, button.image.width, button.image.height }, 0, 0, 0, 0, NPATCH_NINE_PATCH };
    DrawTextureNPatch(button.image, ninePatchInfo1, button.bound, (Vector2) {
        0, 0
    }, 0, RAYWHITE);
    float fontSize =24 * GetRenderHeight()/800.0;
    DrawText(button.text, button.bound.x + button.bound.width/2 - TextLength(button.text)*fontSize/4, button.bound.y+ button.bound.height/2, fontSize, button.text_color);
}
